  ...
  return (
    <div className="w-full min-h-screen p-4 grid gap-4 relative md:w-240 md:min-h-auto md:h-150 md:mx-auto md:mt-16 md:flex md:bg-white rounded-lg">
      <div
        ...
        className="absolute top-0 left-0 p-6 w-full h-40 bg-cover bg-no-repeat bg-center bg-[url('assets/bg-sidebar-mobile.svg')] md:static md:w-72 md:h-full md:bg-[url('assets/bg-sidebar-desktop.svg')] md:rounded-lg"
      >
        <ol className="w-fit p-4 mx-auto text-blue-100 flex items-center gap-4 md:w-full md:flex-col md:gap-8">
          {stepTitles.map((title, index) => (
            <li key={index} className="w-full flex gap-4 items-center">
              <div
                aria-hidden="true"
      ...
      <main
        id="main-content"
        className="w-full flex-1 pt-24 z-10 md:pt-0 flex items-center justify-center"
      >
        {renderStep()}
      </main>
    </div>
